...
class Interpreter(object):
    ...
    def expr(self):
        ...
        while self.current_token.type in (MUL, DIV, PLUS, MINUS):
            ...
            if token.type == MUL:
                self.eat(MUL)
                nums.append(nums.pop() * self.factor())
            elif token.type == DIV:
                self.eat(DIV)
                nums.append(nums.pop() / self.factor())
        ...
        #print nums, ops
        return self.eval_expr(nums, ops)

    def eval_expr(self, nums, ops):
        '''
        evaluate the expression using 2 stacks
        '''
        #print nums, ops
        result = nums[0]
